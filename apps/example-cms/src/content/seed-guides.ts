...
const snapshot = compileSnapshot({
  ...
  definitions: [
    Fields.contentType({
      ...
      guideParagraphText(
        "In the Example CMS, every content type for this blog lives in content/definitions.ts. Posts, authors, categories, tags, comments, and guides share one source of truth. When the model changes, you change one file.",
      ),
  ...
  guideSeedProvidingLayersBody = (): RichText.Document =>
    guideDocument(
      ...
      guideHeading(2, "One file per layer in the Example CMS"),
      guideParagraphText("The reference app splits dependencies into a layers/ folder:"),
      guideUnorderedList([
        guideListItemText("layers/authorization.ts decides who can do what"),
        guideListItemText("layers/identity.ts reads who is making this request"),
        guideListItemText("layers/identifiers.ts generates new IDs"),
        guideListItemText("layers/persistence.ts stores entries and assets"),
        guideListItemText("layers/cms.ts merges everything into Cms.makeLayer"),
      ]),
...
import { layer as authorizationLayer } from "./layers/authorization.ts"
import { layer as identityLayer } from "./layers/identity.ts"
import { layer as persistenceLayer } from "./layers/persistence.ts"
