...
const { canonicalizeJsonValue, parseBody, requiredParameter } = deliverySupport,
  ...
  pendingCommentReceipt = (
    // oxlint-disable-next-line typescript/prefer-readonly-parameter-types -- [EH-257] mutation receipts use discriminated union shapes from CMS operations.
  ...
  validateCommentFields = (
    // oxlint-disable-next-line typescript/prefer-readonly-parameter-types -- [EH-222] comment submission bodies are validated as loosely typed JSON records.
    body: Record<string, unknown>,
  ): Effect.Effect<
