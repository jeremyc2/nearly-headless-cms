...
const assetExtension = (mediaType: string): string => {
  ...
  authorizationHeader = (serviceToken: string): { readonly authorization: string } => ({
    authorization: `Bearer ${serviceToken}`,
  }),
  cmsBaseUrl = Bun.env.EXAMPLE_BLOG_CMS_URL ?? "http://localhost:3001",
  ...
  fetchAsset =
    (serviceToken: string) =>
    (asset: PublicAsset): Effect.Effect<PublicAsset, FetchExportFailure, HttpClient.HttpClient> => {
      ...
      return HttpClient.get(url, { headers: authorizationHeader(serviceToken) }).pipe(
        ...
        Effect.flatMap((contents) =>
          Effect.tryPromise({
            ...
            try: () =>
              Bun.write(stagePath, contents).then(() =>
                Bun.$`mv ${stagePath} ${finalPath}`.quiet(),
              ),
          }),
        ),
  ...
  noItemsCount = 0,
  readPublicExport = (serviceToken: string) =>
    Effect.gen(function* readPublicExportEffect() {
      const cmsClient = makeHeadlessClient(cmsBaseUrl, serviceToken),
        discovery = yield* cmsClient.discover;
      if (discovery.richText.extensions.length > noItemsCount) {
        return yield* UnsupportedDefinition.make({
          message: "Example Blog cannot support the advertised Definition Snapshot",
        });
      }
  ...
  resolveServiceToken = (): Effect.Effect<string, FetchExportFailure, HttpClient.HttpClient> => {
    const configuredToken = Bun.env.EXAMPLE_BLOG_SERVICE_TOKEN;
    if (configuredToken !== undefined && configuredToken.length > 0) {
      return Effect.succeed(configuredToken);
    }
    return HttpClient.get(`${cmsBaseUrl}/development/token/headless`).pipe(
      Effect.mapError((cause) =>
        FetchExportFailure.make({ cause, message: "Service token request failed" }),
      ),
      Effect.flatMap((response) => {
        if (response.status < assetResponseLowerBound || response.status >= assetResponseUpperBound) {
          return FetchExportFailure.make({
            message: `Service token request failed with ${response.status}`,
          });
        }
        return response.json.pipe(
          Effect.mapError((cause) =>
            FetchExportFailure.make({ cause, message: "Service token response decode failed" }),
          ),
          Effect.flatMap((payload) => {
            if (
              ...
              return FetchExportFailure.make({ message: "Service token response was missing token" });
            }
            return Effect.succeed(payload.token);
          }),
        );
      }),
    );
  },
  fetchPublicExportProgram = (serviceToken: string) =>
    Effect.gen(function* fetchPublicExport() {
      const exported = yield* readPublicExport(serviceToken);
      yield* Effect.promise(() => Bun.$`mkdir -p ${generatedDirectory} ${assetsDirectory}`.quiet());
      const assets = yield* Effect.forEach(exported.assets, fetchAsset(serviceToken), {
...
try {
  await zRuntime.runPromise(
    Effect.gen(function* fetchExampleBlogExport() {
      const serviceToken = yield* resolveServiceToken();
      return yield* fetchPublicExportProgram(serviceToken);
    }),
  );
} finally {
