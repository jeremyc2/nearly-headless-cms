...
const apiContractVersion = 1,
  ...
  // oxlint-disable-next-line effecttsgo/missing-pipeable-signature -- [EH-342] generated headless client wrapper stays a plain factory for build-time export.
  zMakeHeadlessClient = (baseAddress: string, bearerToken?: string): HeadlessClient => {
    const generatedClient = makeGeneratedClient(baseAddress),
      requestHeaders = authorizationHeaders(bearerToken);
    return {
      discover: generatedClient
        .discoverPublicDefinitionSnapshot({ headers: requestHeaders })
      ...
      exportPublicBlog: (expectedFingerprint) =>
        generatedClient
          .exportPublicBlog({
            headers: {
              ...requestHeaders,
              "CMS-Expected-Definition-Fingerprint": expectedFingerprint,
            },
          })
          .pipe(
