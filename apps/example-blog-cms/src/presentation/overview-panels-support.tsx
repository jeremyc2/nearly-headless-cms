...
import { draftPluralSuffix, rebuildLabel } from "./main-labels.ts";
...
const OverviewBuildPanel = <Rebuild extends OverviewState["rebuild"]>({
    ...
    <section className="panel">
      ...
      <div className="build-card">
        ...
        {rebuild.isSuccess && <p role="status">The Example Blog static build completed.</p>}
        {rebuild.error && (
  ...
  OverviewSignalGrid = <Counts extends OverviewState["counts"]>({
    ...
    <section className="signal-grid" aria-label="Overview counters">
      ...
      <article className="signal-card">
        ...
        <div>
          <strong>{counts["author"]}</strong>
          <span>Authors</span>
        </div>
        <small>Editorial profiles</small>
      </article>
      <article className="signal-card">
        <span className="signal-icon">◫</span>
        <div>
