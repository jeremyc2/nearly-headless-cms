...
export const toAbortSignal = <SignalType extends ReadonlyTransportAbortSignal>(
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- [EH-214] Web APIs require AbortSignal; transport callers always pass the real signal.
    signal as unknown as AbortSignal,
  toWebRequest = <RequestType extends ReadonlyTransportRequest>(
    request: Readonly<RequestType>,
  ): Request =>
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- [EH-215] Web APIs require Request; transport callers always pass the real request.
