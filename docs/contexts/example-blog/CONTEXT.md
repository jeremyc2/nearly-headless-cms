...
**Comment**:
Content submitted through the Public Blog and related to a Post. A Comment has a display name, plain-text body, optional website URL, creation time, and Comment Status. Comment moderation is expressed through ordinary CMS Builder-defined Fields, not library-defined workflow semantics. Comments do not have replies, visitor editing/deletion, likes, or email notifications.
_Avoid_: Feedback, message

**Comment Status**:
An ordinary CMS Builder-defined field on a Comment whose values include pending, approved, and rejected. Only approved Comments are displayed by the Public Blog.
