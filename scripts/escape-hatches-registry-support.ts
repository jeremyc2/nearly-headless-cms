...
const registryCodeWidth = 3,
  ...
  loadExistingCodes = async (): Promise<ReadonlyMap<string, string>> => {
    ...
    for (const line of document.split("\n")) {
      ...
      if (match?.groups === undefined) {
        // oxlint-disable-next-line eslint/no-continue -- [EH-178] registry assignment skips unresolved rule and code pairs.
        continue;
      }
      // oxlint-disable-next-line eslint/one-var -- [EH-192] registry helpers keep related declarations grouped.
  ...
  // oxlint-disable-next-line eslint/max-statements -- [EH-175] registry formatting keeps comment rendering colocated.
  formatInlineComment = (hatch: EscapeHatch, codes: readonly string[]): string => {
    ...
    // oxlint-disable-next-line eslint/no-ternary, unicorn/prefer-ternary -- [EH-180, EH-280] registry formatting keeps compact comment labels.
  ...
  // oxlint-disable-next-line eslint/max-statements, eslint/max-lines-per-function, effecttsgo/async-function -- [EH-174, EH-168, EH-026] escape hatch registry CLI uses async filesystem IO.
  assignCodes = async (
    hatches: readonly EscapeHatch[],
    options: { readonly reindex?: boolean } = {},
  ): Promise<{
    ...
    // oxlint-disable-next-line eslint/no-ternary, eslint/one-var -- [EH-289, EH-290] reindex mode chooses between an empty code map and the persisted registry map.
    const existingCodeByKey = await (options.reindex === true
    ...
    for (const key of sortedKeys) {
      ...
      // oxlint-disable-next-line eslint/no-ternary -- [EH-284] reindex mode chooses between fresh sequential codes and preserved registry codes.
      let code = options.reindex === true ? undefined : codeByKey.get(key);
      if (code === undefined) {
        // oxlint-disable-next-line eslint/no-ternary -- [EH-284] reindex mode chooses between fresh sequential codes and preserved registry codes.
        code = options.reindex === true ? `EH-${String(assignedCodes.size + 1).padStart(registryCodeWidth, "0")}` : nextRegistryCode(assignedCodes);
  ...
  updateSourceFiles = async (
    ...
    for (const [filePath, fileHatches] of hatchesByFile) {
      ...
      // oxlint-disable-next-line eslint/one-var, eslint/no-ternary -- [EH-191, EH-180] registry formatting keeps compact comment labels.
  ...
  // oxlint-disable-next-line effecttsgo/async-function, effecttsgo/missing-pipeable-signature -- [EH-027, EH-123] escape hatch registry helpers are intentionally direct-call only.
  applyRegistryCodes = async (
    ...
    const { codeByKey, entries } = await assignCodes(hatches, options),
