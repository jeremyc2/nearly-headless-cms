...
// oxlint-disable-next-line eslint/sort-vars -- [EH-259] branch text is trimmed immediately into the release evidence artifact field.
 branch = branchText.trim(),
 bunVersion = Bun.version,
 commitText = await Bun.$`git rev-parse HEAD`.cwd(monorepoRoot).text(),
// oxlint-disable-next-line eslint/sort-vars -- [EH-260] commit text is trimmed immediately into the release evidence artifact field.
 commit = commitText.trim(),
 evidenceJsonIndent = 2,
 generatedAtText = await Bun.$`date -u +%Y-%m-%dT%H:%M:%SZ`.text(),
// oxlint-disable-next-line eslint/sort-vars -- [EH-265] generated-at text is trimmed immediately into the release evidence artifact field.
 generatedAt = generatedAtText.trim(),
 inspectionPath = path.join(artifactsDirectory, "inspection.json"),
// oxlint-disable-next-line eslint/sort-vars -- [EH-268] inspection report parsing follows the resolved archive and git metadata fields.
 inspectionReport: unknown = JSON.parse(await Bun.file(inspectionPath).text()),
 nodeVersion = process.version,
 // oxlint-disable-next-line eslint/sort-vars -- [EH-261] evidence object aggregates the resolved release metadata fields.
 evidence = {
